...
#CAPTURING IMAGE TO PERFORM OPERATION ON IT
while(cap.isOpened()):
    ...
    mask2 = cv2.bitwise_not(mask1) #Expect the clock

    res1= cv2.bitwise_and(background,background,mask=mask1)# used for segmentation of the color
    res2= cv2.bitwise_and(img,img,mask=mask2) #used to substute the cloak part
    final_output = cv2.addWeighted(res1,1,res2,1,0)

    cv2.imshow('SHEKHAR jadui Chola !',final_output)
    k = cv2.waitKey(10)
    if k ==27:
        break
